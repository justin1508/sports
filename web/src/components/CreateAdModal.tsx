...
import * as Checkbok from '@radix-ui/react-checkbox'
      ...
      <Dialog.Content className="fixed bg-[#2a2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[480px] shadow-lg shadow-black/25">
        ...
        <form onSubmit={handleCreateAd} className="mt-8 flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            ...
            <select
              className="bg-zync-900 py-3 px-4 rounded text-sm placeholder:text-zync-500 appearance-none"
              id="game"
              name="game"
              defaultValue=""
            >
          ...
          <div className="flex flex-col gap-2">
            ...
            <Input name="name" id="name" placeholder="Seu nickname" />
          </div>

          <div className="grid grid-cols-2 gap-6">
            <div >
              <label htmlFor="yearsPlaying">Joga há quanto tempo?</label>
              <Input name="yearsPlaying" id="yearsPlaying" type="number" placeholder="XP em anos" />
            </div>
            <div className="flex flex-col gap-2">
          ...
          <label className="mt-2 flex items-center gap-2 text-sm">
            <Checkbok.Root checked={useVoiceChannel} onCheckedChange={(checked) => {
              if (checked === true) {
          ...
          <footer className="mt-4 flex justify-end gap-4">
            <Dialog.Close type="button" className="bg-zinc-500 px-5 h-12 rounded-md font-semibold bg-zinc-600">Cancelar</Dialog.Close>
            <button
